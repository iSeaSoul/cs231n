...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    loss += self.reg * (np.sum(self.params['W1'] * self.params['W1']) + 
          np.sum(self.params['W2'] * self.params['W2'])) * 0.5
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    for layer_idx in xrange(1, self.num_layers):
      input_d = input_dim if layer_idx == 1 else hidden_dims[layer_idx  - 2]
      self.params['W' + str(layer_idx)] = weight_scale * np.random.randn(input_d, hidden_dims[layer_idx - 1])
      self.params['b' + str(layer_idx)] = np.zeros(hidden_dims[layer_idx - 1])
      if self.use_batchnorm:
        self.params['gamma' + str(layer_idx)] = np.ones(hidden_dims[layer_idx - 1])
        self.params['beta' + str(layer_idx)] = np.zeros(hidden_dims[layer_idx - 1])
    input_d = input_dim if self.num_layers == 1 else hidden_dims[self.num_layers - 2]
    self.params['W' + str(self.num_layers)] = weight_scale * np.random.randn(input_d, num_classes)
  ...
  def loss(self, X, y=None):
    ...
    for layer_idx in xrange(1, self.num_layers):
      ...
      if self.use_batchnorm:
        ...
        temp, cache[layer_idx] = affine_relu_dropout_forward(temp, \
              self.params['W' + str(layer_idx)], self.params['b' + str(layer_idx)],\
              self.dropout_param)
      else:
        temp, cache[layer_idx] = affine_relu_forward(temp, \
              self.params['W' + str(layer_idx)], self.params['b' + str(layer_idx)])

    scores, cache[self.num_layers] = affine_forward(temp, \
    ...
    dx, grads['W' + str(self.num_layers)], grads['b' + str(self.num_layers)] = \
          affine_backward(dx, cache[self.num_layers])
    ...
    for layer_idx in range(self.num_layers - 1, 0, -1):
      if self.use_batchnorm:
        ...
        dx, grads['W' + str(layer_idx)], grads['b' + str(layer_idx)] = \
            affine_relu_backward(dx, cache[layer_idx])

      reg_loss += np.sum(self.params['W' + str(layer_idx)] * self.params['W' + str(layer_idx)])
      grads['W' + str(layer_idx)] += self.reg * self.params['W' + str(layer_idx)]
